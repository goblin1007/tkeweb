...
if ($id) {
    $res = $conn->query("SELECT * FROM noidung_hequocte WHERE id = " . intval($id));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($_POST['id']) {
      ...
      $query = "UPDATE noidung_hequocte SET muc='$muc', tieude='$tieude', noidung='$noidung'";
      if ($filename) {
      ...
      // Thêm mới
      $conn->query("INSERT INTO noidung_hequocte (muc, tieude, noidung, hinhanh, ngaydang) VALUES ('$muc', '$tieude', '$noidung', '$filename', NOW())");
  }

  header("Location: adminhequocte.php"); exit();
}
// xóa nè 
if (isset($_GET['delete'])) {
  $delete_id = intval($_GET['delete']);
  $conn->query("DELETE FROM noidung_hequocte WHERE id = $delete_id");
  header("Location: adminhequocte.php"); exit();
}
...
<head>
  <meta charset="UTF-8">
  <title>Quản lý nội dung - Hệ quốc tế</title>
...
<a href="index.php" class="button-link">Quản lý đào tạo</a>
  ...
  <h2>Quản lý nội dung - Hệ quốc tế</h2>
...
<?php if ($edit_data): ?>
  <a href="adminhequocte.php" class="button-link">Huỷ sửa</a>
<?php endif; ?>
  </form>
...
<table border="1" cellpadding="10" cellspacing="0" class= "table">
  ...
  $conn = new mysqli("localhost", "root", "", "daotao");
  $conn->set_charset("utf8");
  $res = $conn->query("SELECT * FROM noidung_hequocte ORDER BY id DESC");
  while ($r = $res->fetch_assoc()) {
    echo "<tr>
            ...
            <td>
              ...
              <a href='?delete={$r['id']}' onclick=\"return confirm('Bạn chắc chắn muốn xoá nội dung này?')\">Xoá</a>
            </td>";
          }      
  ?>
</table>
