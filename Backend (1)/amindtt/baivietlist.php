...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['id'])) {
        ...
        $query = "UPDATE baiviet SET tieu_de=?, mo_ta_ngan=?, theloai=?, chuyenmuc=?, hot=?, noidung=?";
        if ($filename) {
            $query .= ", hinhanh=?";
        }
        $query .= " WHERE id=?";
        ...
        if ($filename) {
            $stmt->bind_param("ssssisis", $tieude, $mo_ta_ngan, $theloai, $chuyenmuc, $hot, $noidung, $filename, $update_id);
        } else {
            $stmt->bind_param("ssssisi", $tieude, $mo_ta_ngan, $theloai, $chuyenmuc, $hot, $noidung, $update_id);
        }
        $stmt->execute();
    } else {
        // Thêm mới bài viết
        $stmt = $conn->prepare("INSERT INTO baiviet (tieu_de, mo_ta_ngan, hinhanh, theloai, chuyenmuc, hot, ngay_dang, noidung) 
                                VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)");
        $stmt->bind_param("ssssiss", $tieude, $mo_ta_ngan, $filename, $theloai, $chuyenmuc, $hot, $noidung);
        $stmt->execute();
    }
...
<form action="" method="POST" enctype="multipart/form-data">
    ...
    <textarea name="mo_ta_ngan" id="mo_ta_ngan"><?= htmlspecialchars($edit_data['mo_ta_ngan'] ?? '') ?></textarea>
    ...
    <input type="text" name="noidung" id="noidung" required value="<?= htmlspecialchars($edit_data['noidung'] ?? '') ?>">
