...
if ($id) {
    $res = $conn->query("SELECT * FROM noidung_saudaihoc WHERE id = " . intval($id));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($_POST['id']) {
      ...
      $query = "UPDATE noidung_saudaihoc SET muc='$muc', tieude='$tieude', noidung='$noidung'";
      if ($filename) {
      ...
      // Thêm mới
      $conn->query("INSERT INTO noidung_saudaihoc (muc, tieude, noidung, hinhanh, ngaydang) VALUES ('$muc', '$tieude', '$noidung', '$filename', NOW())");
  }

  header("Location: adminsaudh.php"); exit();
}
// xóa nè 
if (isset($_GET['delete'])) {
  $delete_id = intval($_GET['delete']);
  $conn->query("DELETE FROM noidung_saudaihoc WHERE id = $delete_id");
  header("Location: adminsaudh.php"); exit();
}
...
<head>
  <meta charset="UTF-8">
  <title>Quản lý nội dung - Sau đại học </title>
...
<?php if ($edit_data): ?>
  <a href="adminsaudh.php" class="button-link">Huỷ sửa</a>
<?php endif; ?>
  </form>
...
<table border="1" cellpadding="10" cellspacing="0" class= "table">
  ...
  $conn = new mysqli("localhost", "root", "", "daotao");
  $conn->set_charset("utf8");
  $res = $conn->query("SELECT * FROM noidung_saudaihoc ORDER BY id DESC");
