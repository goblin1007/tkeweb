...
if ($id) {
    $res = $conn->query("SELECT * FROM noidung_phuongthuc WHERE id = " . intval($id));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($_POST['id']) {
      ...
      $query = "UPDATE noidung_phuongthuc SET muc='$muc', tieude='$tieude', noidung='$noidung'";
      if ($filename) {
      ...
      // Thêm mới
      $conn->query("INSERT INTO noidung_phuongthuc (muc, tieude, noidung, hinhanh, ngaydang) VALUES ('$muc', '$tieude', '$noidung', '$filename', NOW())");
  }

  header("Location: phuongthuc.php"); exit();
}
// xóa nè 
if (isset($_GET['delete'])) {
  $delete_id = intval($_GET['delete']);
  $conn->query("DELETE FROM noidung_phuongthuc WHERE id = $delete_id");
  header("Location: phuongthuc.php"); exit();
}
...
<head>
  <meta charset="UTF-8">
  <title>Quản lý nội dung - Phương thức tuyển sinh </title>
...
<a href="indexts.php" class="button-link">Quản lý Tuyển sinh </a>
  ...
  <h2>Quản lý nội dung - Phương thức tuyển sinh </h2>
...
<table border="1" cellpadding="10" cellspacing="0" class="table">
  ...
  $conn = new mysqli("localhost", "root", "", "tuyensinhtmu");
  $conn->set_charset("utf8");
  $res = $conn->query("SELECT * FROM noidung_phuongthuc ORDER BY id DESC");
